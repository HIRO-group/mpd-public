...
from mpd.utils.loading import load_params_from_yaml, get_start_goal_from_yaml
...
def experiment(
    ...
    **kwargs
    ...
    env = EnvYaml(tensor_args=tensor_args, yaml_file=ENV_YAML, **args)
    ...
    # shelf
    start_state_pos, goal_state_pos = get_start_goal_from_yaml(REQ_YAML)
    start_state_pos = torch.tensor(start_state_pos, **tensor_args).unsqueeze(0)
    goal_state_pos = torch.tensor(goal_state_pos, **tensor_args).unsqueeze(0)
    # start_state_pos = torch.tensor([0, -0.785, 0, -2.356, 0, 1.571, 0.785], **tensor_args).unsqueeze(0)
