...
class BodyNet(nn.Module):
    def __init__(self, env_name, net, action_dim, embedding_dim, ouput_activation=None, global_input=False, fixed_std=0.1, device='cuda'):
        super(BodyNet, self).__init__()
        
        self.std = fixed_std
        
        self.tokenizer = tokenizer(env_name, embedding_dim, ouput_activation=ouput_activation, device=device)
        self.net = net
        self.detokenizer = detokenizer(env_name, net.ouput_dim, action_dim, device=device, global_input=global_input)
